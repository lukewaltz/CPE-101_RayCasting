...
def cast_ray(ray, sphere_list, eye_point, ambient_color, light):
...
def cast_all_rays(min_x, max_x, min_y, max_y, width, height, eye_point, sphere_list, ambient_color, light):
    ...
    for j in range(height):
        for i in range(width):
            ...
            color = cast_ray(eye_ray, sphere_list, eye_point, ambient_color, light)
...
cast_all_rays(-10, 10, -7.5, 7.5, 512, 384, data.point(0, 0, -14),
              [data.sphere(data.point(1, 1, 0), 2, data.color(0.0, 0.0, 1),
                           data.finish(0.2, 0.4)),
               data.sphere(data.point(0.5, 1.5, -3), 0.5, data.color(1, 0.0, 0.0),
                           data.finish(0.4, 0.4))],

...
def color_mult(c1, finish, c3):
    return data.color((c1.R * finish * c3.R), (c1.G * finish * c3.G), (c1.B * finish * c3.B))


def cast_ray(ray, sphere_list, eye_point, ambient):
    ...
    if lst != []:
        ...
        for t in lst:
            if distance(eye_point, t[0].center) < d:
                ...
                color = color_mult(t[0].color, t[0].finish, ambient)
    return color


def cast_all_rays(min_x, max_x, min_y, max_y, width, height, eye_point, sphere_list, ambient):
    print("P3")
    print(width, height)
    print("255")
    ...
    for j in range(height):
        for i in range(width):
            ...
            color = cast_ray(eye_ray, sphere_list, eye_point, ambient)
            print(color)
